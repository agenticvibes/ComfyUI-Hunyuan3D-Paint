...
class MeshRender:
    def __init__(
        ...
        if camera_type == "orth":
            self.set_orth_scale(ortho_scale)
    ...
    def set_mesh(self, vtx_pos, pos_idx, vtx_uv=None, uv_idx=None, scale_factor=1.15, auto_center=True):
        ...
        self.pos_idx = torch.from_numpy(pos_idx).int().to(self.device)

        if (vtx_uv is not None) and (uv_idx is not None):
            self.vtx_uv = torch.from_numpy(vtx_uv).float().to(self.device)
            self.uv_idx = torch.from_numpy(uv_idx).int().to(self.device)
    ...
    def extract_textiles(self):
        ...
        vertex_normals = torch.from_numpy(vertex_normals).float().to(self.vtx_pos).contiguous()
    ...
    def back_project(self, image, elev, azim, camera_distance=None, center=None, method=None):
        ...
        if method == "linear":
            ...
            ).float().to(self.tex_position)
            w2c = torch.from_numpy(r_mv).float().to(self.tex_position)
            ...
            # Use hash set for memory-efficient isin (avoids O(N*M) expansion on MPS)
            if valid_idx.device.type == "mps":
                inner_set = set(inner_valid_idx.cpu().tolist())
                intersection_mask = torch.tensor([v.item() in inner_set for v in valid_idx.cpu()], dtype=torch.bool, device=valid_idx.device)
            else:
