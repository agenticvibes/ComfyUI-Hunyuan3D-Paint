...
def _chunked_scaled_dot_product_attention(query, key, value, attn_mask=None, chunk_size=1024):
    """Memory-efficient attention via online softmax (Rabe & Staats 2021).
    O(chunk_size) memory instead of O(seq_len²). Mathematically equivalent to SDPA.
    Accumulation in float32 for numerical stability with float16 inputs."""
    ...
    # Accumulate in float32 to avoid precision loss over many chunks
    query = query.float()
    key = key.float()
    value = value.float()

    output = torch.zeros(B, H, N_q, D_v, device=query.device, dtype=torch.float32)
    running_max = torch.full((B, H, N_q, 1), float('-inf'), device=query.device, dtype=torch.float32)
    running_sum = torch.zeros((B, H, N_q, 1), device=query.device, dtype=torch.float32)

    for j in range(0, N_kv, chunk_size):
        k_chunk = key[:, :, j:j + chunk_size]
        v_chunk = value[:, :, j:j + chunk_size]

        scores = torch.matmul(query, k_chunk.transpose(-2, -1)) * scale

        if attn_mask is not None:
            scores = scores + attn_mask[:, :, :, j:j + chunk_size].float()
        ...
        output = output + torch.matmul(exp_scores, v_chunk)
        running_sum = running_sum + exp_scores.sum(dim=-1, keepdim=True)

        running_max = new_max

    return (output / running_sum).to(input_dtype)
...
class AttnCore:
    ...
    def process_attention_base(
        ...
        **kwargs,
        ...
        # Compute attention — use chunked implementation on MPS for large sequences to avoid O(n²) OOM
        if query.device.type == "mps" and query.shape[2] > 8192:
            hidden_states = _chunked_scaled_dot_product_attention(
                query, key, value, attn_mask=attention_mask, chunk_size=1024
            )
        else:
